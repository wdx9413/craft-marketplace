...
The recommended sibling plugins are optional standalone projections of the same runtime: install `craft-knowledge` or `craft-memory` for governed context, `craft-capability` for discovery, `craft-quality` for repeatable evaluation, or `craft-workflow-evolution` for bounded draft proposals. They should be installed *instead of* the complete plugin when a Host needs only that bounded outcome. `craft-workflow-evolution` is a draft helper, not a complete standalone evolution loop. Compare `craft_info.data_space_id` before assuming two components share one ledger, and avoid enabling overlapping siblings alongside `craft` unless the Host explicitly needs duplicate direct tools.
...
Craft Studio is the separately packaged desktop companion. It serves the same local Craft runtime in a native app-style shell; install this plugin for MCP integration, and obtain the desktop bundle from the source repository's release artifacts when a visual workbench is needed.
