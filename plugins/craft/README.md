...
The recommended sibling plugins are optional standalone projections of the same runtime: install `craft-context` for governed knowledge and memory, `craft-capability` for discovery, or `craft-quality` for repeatable evaluation. They should be installed *instead of* the complete plugin when a Host needs only that bounded outcome. `craft-knowledge`, `craft-memory`, and `craft-skill-quality` remain compatible narrow/legacy projections. `craft-experience` is a draft helper, not a complete standalone evolution loop. Compare `craft_info.data_space_id` before assuming two components share one ledger, and avoid enabling overlapping siblings alongside `craft` unless the Host explicitly needs duplicate direct tools.
